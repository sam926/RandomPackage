...
public class EnchantsThatAddPotionEffects implements Listener {
	...
	private void inventoryClickEvent(InventoryClickEvent event) {
		if(event.isCancelled() || !(event.getSlotType() == SlotType.ARMOR)) {
			...
			return;
		} else if(event.getRawSlot() == 5 && !(event.getCursor() == null) && event.getCursor().hasItemMeta() && event.getCursor().getItemMeta().hasLore() && event.getCursor().getType().name().endsWith("HELMET")
				|| event.getRawSlot() == 6 && !(event.getCursor() == null) && event.getCursor().hasItemMeta() && event.getCursor().getItemMeta().hasLore() && event.getCursor().getType().name().endsWith("CHESTPLATE")
				|| event.getRawSlot() == 7 && !(event.getCursor() == null) && event.getCursor().hasItemMeta() && event.getCursor().getItemMeta().hasLore() && event.getCursor().getType().name().endsWith("LEGGINGS")
				|| event.getRawSlot() == 8 && !(event.getCursor() == null) && event.getCursor().hasItemMeta() && event.getCursor().getItemMeta().hasLore() && event.getCursor().getType().name().endsWith("BOOTS")) {
			for(int i = 1; i <= 4; i++) {
				// Legendary Enchants //
				/*
				 * Drunk
